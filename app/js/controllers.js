...
angular.module('myApp.controllers', [])
  ...
  .controller('MyCtrl2', ['$scope', 'Isis', function($scope, Isis) {
	// Instantiate an object to store your scope data in (Best Practices)
	    $scope.data = {};
	    
	    Isis.query(function(response) {
	        // Assign the response INSIDE the callback
	        $scope.data = response;
	      });
  }])
  .controller('MyCtrl3', ['$scope', 'Hello', function($scope, Hello) {
	// Instantiate an object to store your scope data in (Best Practices)
	    $scope.data = {};
	    
	    Hello.get(function(response) {
	        // Assign the response INSIDE the callback
	        $scope.data = response;
	      });
  }]);